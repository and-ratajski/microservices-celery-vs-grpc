...
Celery Worker B in microservices-based celery architecture.
...
```shell
celery call tasks.task_b --args='["arg_1", "arg_2", ...]' --kwargs='{"kwarg_1": kwarg_value, ...}'
```

2. via [Flower](../CeleryInfrastructure) REST API

```shell
curl -X POST --location 'http://localhost:8080/api/task/send-task/tasks.task_b' \
